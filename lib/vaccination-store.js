...
export function replaceEncryptedRecord(envelopes, id, value, secret = configuredSecret(), now = new Date()) {
  const index = envelopes.findIndex(envelope => envelope.id === id);
  if (index === -1) return null;
  const existing = decryptRecord(envelopes[index], secret);
  const record = {
    ...value,
    id,
    logged_at: existing.logged_at || now.toISOString(),
    updated_at: now.toISOString(),
  };
  const records = envelopes.slice();
  records[index] = encryptRecord(record, secret);
